...
const inboxShowAll = ref(false)

const sortedInbox = computed(() =>
  [...store.inbox].sort((a, b) => {
    if (a.llm_score == null && b.llm_score == null) return 0
    if (a.llm_score == null) return 1
    if (b.llm_score == null) return -1
    return b.llm_score - a.llm_score
  })
)

const visibleInbox = computed(() =>
  inboxShowAll.value ? sortedInbox.value : sortedInbox.value.slice(0, 10)
)
...
<template>
  <section class="space-y-4">
    ...
    <p v-if="lastRunMsg" class="rounded bg-emerald-50 p-3 text-sm text-emerald-700">{{ lastRunMsg }}</p>
    ...
    <div class="bg-white shadow rounded-lg p-6 space-y-4">
      ...
      <div v-if="sortedInbox.length > 10" class="text-center">
        <button
          ...
          @click="inboxShowAll = !inboxShowAll"
        >
          {{ inboxShowAll ? '折叠' : `显示全部（共 ${sortedInbox.length} 条）` }}
        </button>
      </div>
    </div>
      ...
      <div v-if="modalOpen" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative z-10 w-full max-w-2xl rounded-lg bg-white shadow-xl mx-4">
          ...
          <form @submit.prevent="submit" class="space-y-3 p-4 text-sm">
            ...
            <!-- ② 类型相关字段：必填的硬目标（DOI / 作者ID / arXiv 分类） -->
            ...
            <!-- ③ 研究兴趣描述（所有类型必填，LLM 用它生成检索式 + 用它评分） -->
            <div>
