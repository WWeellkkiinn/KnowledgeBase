...
**目标**：调用 LLM（OpenAI 兼容中转 API），对论文标题 + 摘要进行分析，自动生成语义标签。
...
| 项 | 细节 |
|----|------|
| LLM 端点 | `CHAT_API_BASE` / `CHAT_API_KEY` / `CHAT_MODEL` 环境变量 |
...
- 与 F1 **合并为单次 LLM 调用**，Prompt 同时返回标签 + 精炼（JSON 格式），减少请求次数
...
定时任务（每日 00:00，APScheduler）
  ...
  → 对每篇论文：调用 LLM 判断与 ABM 领域的相关性（0–1 分）
