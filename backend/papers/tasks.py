...
_EXTRACT_REFS = _ROOT / "scripts" / "extract_refs.py"
...
def process_upload(self, paper_id: int) -> dict:
    ...
    try:
        ...
        # 1) PDF → MD (import lazily; Pdf2MdService is in legacy services/)
        sys.path.insert(0, str(_ROOT))
        from services.pdf2md_service import Pdf2MdService  # type: ignore
        result = Pdf2MdService().convert(pdf_path, output_dir=output_dir)
        if "error" in result:
        ...
        if not paper.doi and paper.title:
            try:
                from services.doi_resolver import resolve_doi  # type: ignore
