...
def extract(md_path: Path, section: int | None = None) -> list[dict]:
    """Parse a Markdown file and return the list of cited references.

    Importable alternative to the CLI entrypoint. Callers in the SaaS Django
    stack should use this instead of subprocess.
    """
    ...
    if section is not None:
        text = _read_section(md_path, section)
        if text == "":
            raise IndexError(f"section_id {section} out of range")
    else:
    ...
    if style == "numbered":
        ...
        for i, entry in enumerate(ref_entries):
            ...
            if section is None or key in cited_keys:
...
def main():
    ...
    try:
        results = extract(Path(args.md_path), section=args.section)
    except FileNotFoundError as exc:
        print(json.dumps({"error": f"File not found: {exc}"}))
        sys.exit(1)
